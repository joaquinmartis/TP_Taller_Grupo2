...

{
     ...
     * @param iVista Contenedor donde se buscara el componente
     * @param class1 nombre del componente a buscar
     * @return devuelve el componente con el nombre indicado. Si no hay ningun componente con el nombre buscado, se devolvera null
     */
    public static Component getComponentForName(Component component, String class1) {
        Component retorno = null;
        if (component.getName() != null && component.getName().equals(class1))
            retorno = component;
        else
        {
            if (component instanceof Container)
            {
                Component[] hijos = ((Container) component).getComponents();
                int i = 0;
                while (i < hijos.length && retorno == null)
                {
                    retorno = getComponentForName(hijos[i], class1);
                    i++;
                }
            }
        }
